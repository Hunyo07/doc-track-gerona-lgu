    ...
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
      <div class="mt-3">
        ...
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-lg font-medium text-gray-900">Edit Issue</h3>
          <button
            @click="$emit('close')"
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                required
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                placeholder="reporter.email@example.com"
              />
          ...
          <div v-if="canUpdateStatus">
            ...
            ></textarea>
            <p v-if="errors.admin_notes" class="mt-1 text-sm text-red-600">{{ errors.admin_notes[0] }}</p>
          </div>
            ...
            <div class="space-y-2">
              ...
              >
                <div class="flex items-center">
                  ...
                  <span class="text-sm text-gray-700">{{ attachment.original_name || attachment.name }}</span>
...
const form = reactive({
  ...
  reporter_name: props.issue.reporter_name || '',
  reporter_email: props.issue.reporter_email || '',
  reporter_phone: props.issue.reporter_phone || '',
...
const submitForm = async () => {
  ...
  try {
    ...
    if (selectedFiles.value.length > 0) {
      issueData.attachments = selectedFiles.value
    }
    
    const updatedIssue = await issueStore.updateIssue(props.issue.id, issueData)
    emit('updated', updatedIssue)
  } catch (error) {
    if (error.response?.data?.errors) {
      errors.value = error.response.data.errors
    } else {
      submitError.value = error.message || 'Failed to update issue. Please try again.'
    }
  } finally {
